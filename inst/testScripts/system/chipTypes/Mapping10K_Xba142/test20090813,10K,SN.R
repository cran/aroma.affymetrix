...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
sn <- ScaleNormalization3(csC);
print(sn);
csN <- process(sn, verbose=log);
print(csN);
stopifnot(identical(getNames(csN), getNames(csC)));

YC <- extractMatrix(csC);
YN <- extractMatrix(csN);
cc <- 1;
plot(YC[,cc], YN[,cc]);
stop("Not implemented correctly: Only a subset of the signals are rescaled. /HB 2009-08-13");
